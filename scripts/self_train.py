...
class Selfplay:
    def __init__(self, model_path, exp_name, char):
        self.config = {
            "iso_path": args.iso,
        ...
        self.timesteps = 0
        self.models = deque(maxlen=5)
        self.models.append(os.path.join(self.save_dir, "agent_0.pt"))
        
    def train(self, cpu=None, actor_model=None):
        self.config["players"] = [MyAgent(getattr(enums.Character, self.char)), MyAgent(getattr(enums.Character, self.char))]
        self.config["actor_model"] = actor_model
        env = gym.vector.AsyncVectorEnv([
            lambda: self.make_selfplay_env(self.config),
            lambda: self.make_cpu_env(self.config, 6),
        ...
        trainer = SequentialTrainer(cfg=cfg_trainer, env=env, agents=learner_agent)
    ...
    def run(self):
        op_model = random.sample(self.models, 1)[0]
        self.train(actor_model=op_model)
        self.kill_dolphin()

if __name__ == "__main__":
    model_path = "/home/tgkang/saved_model/aginst_cpu_FD/MARIO_cpu.pt"
    s = Selfplay(model_path=model_path, exp_name="MARIOSelfplay", char="MARIO")
    for _ in range(10000):
        # for safe executing
        with ProcessPoolExecutor(max_workers=1) as executor:
            futures = []
            futures.append(executor.submit(s.run))
        # add timesteps, copy to recent model, determine save or not
